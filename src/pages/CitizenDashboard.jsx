...
function buildAccessRecord(project) {
  return {
    ...project,
    dharaReference:
      project.rehabilitationDharaId || project.dharaReference || project.projectId || `DH-2026-${String(project.id).slice(-6)}`,
    accessCode:
      project.accessCode ||
    ...
    rehabilitationDharaId: project.rehabilitationDharaId || project.landownerAccess?.dharaId || "",
    rehabilitationReview: project.rehabilitationReview || {},
  }
}
...
function CitizenDashboard() {
  ...
  const handleVerify = async () => {
    ...
    if (!enteredReference) {
      setVerificationError("Enter your DHARA ID to access the landowner record.")
      return
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/landowner/access`, {
        ...
        body: JSON.stringify({
          ...
          surveyNumber: enteredSurvey || undefined,
          accessCode: enteredCode || undefined,
        }),
      })
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <p className="font-mono text-[9px] leading-5 text-[var(--ink-soft)] mt-2 max-w-xl">
                          Enter the DHARA ID issued by the Rehabilitation Authority. Survey and notice details are optional for R&R records.
                        </p>
                      </div>
                  ...
                  <div className="p-5 md:p-7 space-y-5">
                    ...
                    <div>
                      <label className="font-mono text-[8px] uppercase tracking-[0.16em] text-[var(--ink-soft)]">
                        DHARA ID / Reference Number
                      </label>

                      <input
                        value={reference}
                        onChange={(event) => setReference(event.target.value)}
                        placeholder="e.g. DHARA-RR-2026-123456"
                    ...
                    <div>
                      <label className="font-mono text-[8px] uppercase tracking-[0.16em] text-[var(--ink-soft)]">
                        Notice Access Code (optional)
                      </label>

                      <input
                        value={accessCode}
                        onChange={(event) => setAccessCode(event.target.value)}
                        placeholder="Optional for Rehabilitation DHARA ID"
                        className="w-full mt-2 border border-[var(--line)] bg-[var(--paper)] px-4 py-3 outline-none font-mono text-xs focus:border-[var(--earth)]"
                      />
                    </div>
                  ...
                  <section className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-6">
                    ...
                    <div>
                      ...
                      <div className="border border-[var(--line)] bg-[var(--white)]">
                        ...
                        {selectedProject.rehabilitationDharaId && (
                          <div className="border-t border-[var(--line)] bg-emerald-50/60 p-5 md:p-7">
                            <div className="flex items-start gap-3">
                              <ShieldCheck size={18} className="mt-0.5 shrink-0 text-emerald-700" />
                              <div>
                                <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-emerald-700">
                                  Rehabilitation & Resettlement
                                </p>
                                <h3 className="mt-1 font-serif text-xl">Landowner consultation record</h3>
                              </div>
                            </div>
                            <div className="mt-5 grid gap-3 md:grid-cols-3">
                              <div className="border border-emerald-200 bg-white p-4">
                                <p className="font-mono text-[8px] uppercase tracking-[0.15em] text-slate-500">DHARA R&R ID</p>
                                <p className="mt-2 break-all font-mono text-xs font-bold text-emerald-800">{selectedProject.rehabilitationDharaId}</p>
                              </div>
                              <div className="border border-emerald-200 bg-white p-4">
                                <p className="font-mono text-[8px] uppercase tracking-[0.15em] text-slate-500">Landowner consent</p>
                                <p className="mt-2 font-mono text-xs">{selectedProject.rehabilitationReview?.consentPercentage || selectedProject.landownerAccess?.consentPercentage || 0}%</p>
                              </div>
                              <div className="border border-emerald-200 bg-white p-4">
                                <p className="font-mono text-[8px] uppercase tracking-[0.15em] text-slate-500">R&R status</p>
                                <p className="mt-2 font-mono text-xs">{selectedProject.rehabilitationReview?.landownerAgreement || selectedProject.landownerAccess?.agreementStatus || "Under review"}</p>
                              </div>
                            </div>
                            <div className="mt-3 grid gap-3 md:grid-cols-2">
                              <div className="border border-emerald-200 bg-white p-4">
                                <p className="font-mono text-[8px] uppercase tracking-[0.15em] text-slate-500">Meeting notes</p>
                                <p className="mt-2 text-xs leading-6">{selectedProject.rehabilitationReview?.faceToFaceNotes || "No consultation notes recorded."}</p>
                              </div>
                              <div className="border border-emerald-200 bg-white p-4">
                                <p className="font-mono text-[8px] uppercase tracking-[0.15em] text-slate-500">Rehabilitation plan</p>
                                <p className="mt-2 text-xs leading-6">{selectedProject.rehabilitationReview?.resettlementPlan || "No resettlement plan recorded."}</p>
                              </div>
                            </div>
                            <div className="mt-3 grid gap-3 md:grid-cols-3">
                              <div className="border border-emerald-200 bg-white p-4">
                                <p className="font-mono text-[8px] uppercase tracking-[0.15em] text-slate-500">Compensation / funding</p>
                                <p className="mt-2 text-xs leading-6">{selectedProject.compensationFundingDetails || selectedProject.compensationAmount || "To be determined by the authority"}</p>
                              </div>
                              <div className="border border-emerald-200 bg-white p-4">
                                <p className="font-mono text-[8px] uppercase tracking-[0.15em] text-slate-500">Acquisition information</p>
                                <p className="mt-2 text-xs leading-6">{selectedProject.acquisitionPurpose || "Land acquisition information is under review."}</p>
                              </div>
                              <div className="border border-emerald-200 bg-white p-4">
                                <p className="font-mono text-[8px] uppercase tracking-[0.15em] text-slate-500">Approved by</p>
                                <p className="mt-2 text-xs leading-6">{selectedProject.rehabilitationReview?.approvedBy || "Rehabilitation Authority"}</p>
                              </div>
                            </div>
                          </div>
                        )}
